...
**✅ Покрытие символов:** 100% основных функций Tor - ВСЕ symbols resolved! (v1.0.46 - CRITICAL UPDATE!)

**🚨 ВАЖНО v1.0.46:** Обнаружено что `Package.swift` использует `.binaryTarget` → SPM **КОПИРУЕТ готовый binary**, **НЕ компилирует** Tor! Патч crypto_rand_fast.c ВЕРИФИЦИРОВАН В ИСХОДНИКАХ, но **binary в репозитории БЕЗ патча**. Для получения binary С ПАТЧЕМ требуется **ЛОКАЛЬНАЯ СБОРКА**! См. `README_v1_0_46.md` и `CRITICAL_v1_0_46_BUILD_INSTRUCTIONS.md`.

**✅ v1.0.46 добавлено:** `orconfig.h` для упрощения компиляции! Патч верифицирован! Полная документация локальной сборки. verify_patch.sh для проверки. Device (platform 2) + Simulator (platform 7). ПРАВИЛЬНАЯ обработка `INHERIT_RES_KEEP`. **ТРЕБУЕТСЯ ЛОКАЛЬНАЯ СБОРКА или ФОРК!**
...
let dependencies = Dependencies(
    swiftPackageManager: SwiftPackageManagerDependencies([
        .remote(
            ...
            requirement: .upToNextMajor(from: "1.0.36")
        )
    ])
)
```
...
```swift
dependencies: [
    .package(url: "https://github.com/YOU/TorFrameworkBuilder.git", from: "1.0.36")
],
targets: [
