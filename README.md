...
**✅ Покрытие символов:** 100% основных функций Tor - ВСЕ symbols resolved! (v1.0.43 - FIXED crypto_rand_fast.c!)

**🎉 PERMANENT FIX:** Исправлен патч crypto_rand_fast.c! Теперь использует условную проверку `INHERIT_RES_KEEP` вместо неправильного `INHERIT_RES_ALLOCATED`. Применяется автоматически через `fix_conflicts.sh`! Tor 0.4.8.19 запускается на iOS БЕЗ крашей!

**✅ v1.0.43:** КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ патча! Device (platform 2) + Simulator (platform 7). vtool workaround для simulator. ПРАВИЛЬНАЯ обработка `INHERIT_RES_KEEP` на iOS!
